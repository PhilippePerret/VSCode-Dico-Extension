    ...
    def export_for_antidote
      begin
        ...
        .each do |row|
          ...
          #
          # === MISE EN FORME SPÉCIALE ANTIDOTE/RELECTURE ===
          #
          ...
          def diminue(mot)
            # Version simplifiée suffisante 
            mot.downcase.split(/([ -])/).map{|m| if m.length > 1 then "#{m[0]}." else m end}.join('').gsub(/ /, '')
          end
          # - Composition de la sortie
          @ref.puts "#{row["entree"]}"
          @ref.puts "#{table_genres[row["genre"]]}"
